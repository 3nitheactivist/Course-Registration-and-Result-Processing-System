...
//       message.error(
//         "Course Code is missing! Ensure the form field is correct."
//       );
//       setLoading(false);
//       return;
//     }
...
//       } else {
//         // 🔍 Convert courseCode to lowercase before checking
//         const normalizedCourseCode = values.courseCode.toLowerCase();

//         // 🔍 Check if the course already exists in Firestore
...
//           console.log("🚨 Duplicate Course Found! Showing error message...");
//           setTimeout(
//             () => message.error("This course is already registered!"),
//             100
...
//         // ✅ If the course doesn't exist, register it
//         await addDoc(courseRef, {
//           courseTitle: values.courseTitle,
//           courseCode: normalizedCourseCode, // Store as lowercase
...
//           const batchPromises = result.data.map((course) =>
//             addDoc(courseCollection, {
...
//         if (!course[field]) {
//           message.error(`Missing required field: ${field} in CSV file.`);
//           return false;
//         }
//       }
//     }
...
//   //   if (!values.courseCode) {
//   //     setAlertMessage("Course Code is missing! Ensure the form field is correct.");
//   //     setAlertType("error");
//   //     setLoading(false);
//   //     return;
//   //   }
...
//   //   } catch (error) {
//   //     console.error("❌ Error adding course:", error);
//   //     setAlertMessage("An error occurred while creating the course. Please try again.");
//   //     setAlertType("error");
//   //     setSubmitStatus("error");
//   //   } finally {
//   //     setLoading(false);
...
//           setAlertMessage("This course is already registered!");
//           setAlertType("error");
//           setTimeout(() => setAlertMessage(null), 5000); // Keep the alert visible for 5s
//           setLoading(false);
//           return;
//         }
...
//         // ✨ Clear input fields after success
//         form.resetFields();
//         setTimeout(() => setAlertMessage(null), 5000); // Keep the success alert visible for 5s
...
//         transform: (value) => value.trim().replace(/['"]+/g, ""), // Normalize values
//         complete: async (result) => {
...
//             const batchPromises = result.data.map(async (course) => {
//               const {
//                 courseTitle,
//                 courseCode,
//                 creditHours,
...
//                 !level
//               ) {
//                 console.error("🚨 Invalid Data:", course);
//                 setAlertMessage(
//                   "CSV contains missing fields. Please check and try again."
//                 );
//                 setAlertType("error");
//                 return;
//               }
...
//               const querySnapshot = await getDocs(q);
//               if (!querySnapshot.empty) {
//                 console.log(`🚨 Duplicate course found: ${courseCode}`);
//                 return; // Skip adding duplicate courses
//               }

//               return addDoc(courseCollection, {
...
//             await Promise.all(batchPromises);
//             setAlertMessage("CSV Upload Successful!");
//             setAlertType("success");
...
//           } catch (error) {
//             console.error("❌ Error uploading CSV:", error);
//             setAlertMessage("Failed to upload courses. Try again.");
//             setAlertType("error");
//           } finally {
//             setLoading(false);
//           }
...
//     reader.readAsText(csvFile); // Ensure the file is read as text
//   };
...
const CreateCourse = () => {
  ...
  //     if (!querySnapshot.empty) {
  //       console.log("🚨 Skipping duplicate course:", values.courseCode); // Debugging
  //       setAlertMessage(`❌ Course "${values.courseTitle}" is already registered!`);
  //       setAlertType("error");
  //       setLoading(false);
  //       return;
  //     }
  ...
  //       header: true,
  //       skipEmptyLines: true,
  //       transformHeader: (header) => header.trim().toLowerCase().replace(/['"]+/g, ""), // Normalize to lowercase
  //       transform: (value) => value.trim().replace(/['"]+/g, ""),
  //       complete: async (result) => {
  //         console.log("📂 Parsed CSV Data:", result.data); // Debugging
