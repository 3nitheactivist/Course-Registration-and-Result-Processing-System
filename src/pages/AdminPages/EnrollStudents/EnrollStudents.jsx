...
const EnrollStudents = () => {
  ...
  const handleEnrollStudent = async (values) => {
    ...
    try {
      ...
      const selectedCourseIDs = values.courses; // now using course IDs directly
    ...
    Papa.parse(csvFile, {
      ...
      complete: async (result) => {
        try {
          ...
          for (const student of result.data) {
            const { name, matricNumber, email, department, courses: courseNames } = student;
            if (!name || !matricNumber || !email || !department) {
              showAlert("⚠ Some fields are missing in the CSV file.", "warning");
              continue;
            }
            ...
            const selectedCourseIDs = courseNames.split(",").map(
              (courseTitle) =>
                courses.find(
                  (course) => course.title.trim() === courseTitle.trim()
                )?.id || courseTitle.trim()
  ...
  return (
    <AdminLayout>
      ...
      <Space style={{ marginBottom: 16 }}>
        ...
        <Upload beforeUpload={(file) => setCsvFile(file)} showUploadList={false}>
          <Button icon={<UploadOutlined />}>Upload CSV</Button>
        </Upload>
        <Button type="primary" onClick={handleCSVUpload} disabled={!csvFile}>
          Process CSV
      ...
      {loading ? (
        ...
        <Table columns={columns} dataSource={students} pagination={{ pageSize: 5 }} />
      )}
